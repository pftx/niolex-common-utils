...
public class OSInfo implements Invokable {
    ...
    private enum OsType {
        ...
        public static OsType parseOsType(String s) {
            ...
            for (OsType os : OsType.values()) {
                if (s.startsWith(os.name().toLowerCase())) {
                    return os;
                }
            }
            return LINUX;
    ...
    public OSInfo() {
        ...
        osType = OsType.parseOsType(osName);
    }
    ...
    /**
     * Get Mac CPU Info, Memory Info, and Swap Info all at the same time.
     *
     * @return the information from top result
     */
    public String[] getMacCpuAndMemInfo() {
        Scanner scan = null;
        try {
            Process process = Runtime.getRuntime().exec("top -l 1 -s 0");
            ...
            scan.nextLine();
            scan.nextLine();
            return new String[] {s1, scan.nextLine(), scan.nextLine()};
